 ...
 * **Pithos backend**:
   *snf-image* contains a special command-line tool (*pithcat*) for retrieving
   ...
   to setup ``PITHOS_DATA`` and ``PITHOS_DB`` variables, and when ``rados`` is
   used the user needs to setup ``PITHOS_RADOS_POOL_MAPS`` and
   ``PITHOS_RADOS_POOL_BLOCKS`` accordingly.
